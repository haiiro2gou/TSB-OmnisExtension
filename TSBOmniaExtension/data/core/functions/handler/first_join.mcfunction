...
# お友達(概念)
    tag @s add Friend
# 無信仰にする
    execute unless predicate player_manager:is_believe/flora unless predicate player_manager:is_believe/nyaptov unless predicate player_manager:is_believe/rumor unless predicate player_manager:is_believe/urban unless predicate player_manager:is_believe/wi-ki unless predicate player_manager:is_believe/omnia run function player_manager:god/none/set_tag
# ステータス初期化
